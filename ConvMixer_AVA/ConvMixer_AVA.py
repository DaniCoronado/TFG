...
x_train = tf.data.Dataset.from_tensor_slices((avaRegression.train_image_paths, avaRegression.train_scores)).map(parse_image).map(preprocess).shuffle(256).batch(16).prefetch(-1)
x_test = tf.data.Dataset.from_tensor_slices((avaRegression.test_image_paths, avaRegression.test_scores)).map(parse_image).map(preprocess).batch(16).prefetch(-1)
x_val = tf.data.Dataset.from_tensor_slices((avaRegression.val_image_paths, avaRegression.val_scores)).map(parse_image).map(preprocess).batch(16).prefetch(-1)
