...
x_train = tf.data.Dataset.from_tensor_slices((avaBinaryClasses.train_image_paths, avaBinaryClasses.train_scores)).map(parse_image).map(preprocess).shuffle(1024).batch(64).prefetch(-1)
x_test = tf.data.Dataset.from_tensor_slices((avaBinaryClasses.test_image_paths, avaBinaryClasses.test_scores)).map(parse_image).map(preprocess).batch(64).prefetch(-1)
x_val = tf.data.Dataset.from_tensor_slices((avaBinaryClasses.val_image_paths, avaBinaryClasses.val_scores)).map(parse_image).map(preprocess).batch(64).prefetch(-1)
